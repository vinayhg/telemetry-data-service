...
* 
*/
package com.philips.ps.prs.telemetrydata.controller.hsdpregion;
...
@RestController
public class TelemetryDataHsdpRegionController {
    private static final Logger LOGGER = Logger.getLogger(TelemetryDataHsdpRegionController.class);
    @Autowired
    private RestTemplate restTemplate;
    @Autowired
    private IamTokenHelper iamTokenHelper;
    @Value("${TelemetryDataGlobalRegionUrl}")
    private String TelemetryDataGlobalRegionUrl;
    ...
    public ResponseEntity<Object> saveTelemetryData(@RequestBody @Valid TelemetryDataRequest telemetryDataRequest,
        ...
        try {
            final String requestUrl = TelemetryDataGlobalRegionUrl.concat("/telemetry");
    ...
    public ResponseEntity<TelemetryDataRequest> fetchTelemetryData(
        ...
        try {
            final UriComponents uriComponents = UriComponentsBuilder.newInstance().scheme("https")
                                            .host(TelemetryDataGlobalRegionUrl.concat("/telemetry")).path("/" + nameTag + "/" + productType).build();
    ...
    private HttpHeaders setHeaders() {
        ...
        headers.setBearerAuth(iamTokenHelper.getAccessToken());
        return headers;
    }
}
